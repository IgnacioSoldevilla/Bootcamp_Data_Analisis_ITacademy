...
-- Query principal con EXISTS
SELECT c.company_name AS compañia, c.id AS cod_identificador
FROM company AS c
WHERE EXISTS (
    SELECT t.company_id
    FROM transaction AS t
...
WHERE (c.country = (SELECT c.country
	...
	AND (c.company_name <> "Non Institute") -- ENTIENDO QUE SOLO HAY QUE LISTAR LAS COMPAÑIAS COMPETENCIA Y POR ESO QUITO LA "Non Institute"
    AND (t.declined = 0)
;
